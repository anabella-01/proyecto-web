...
async function gestServices(){
    const response = await fetch("http://localhost:3000/services");
    const services = await response.json();

    services.forEach((service) => createServiceItem(
        service.image,
        service.title,
        service.description,
        "Learn More",
...
async function gestTestimonials(){
    const response     = await fetch("http://localhost:3000/testimonials");
    const testimonials = await response.json();

    testimonials.forEach((testimonial) => createTestimonialItem(
        testimonial.author,
        testimonial.image,
        testimonial.area,
...
function createTestimonialItem(author,image,area,testimonial){
    ...
    const testimonialstemContainer = document.createElement("div");
    testimonialstemContainer.classList.add("testimonials-item");
    ...
    testimonialstemContainer.append(paragraph,testimonialAuthorContainer);

    testimonialsList.append(testimonialstemContainer);
}
